...
class SdfReader:
    ...
    def create_reader(filename):
        if filename.endswith(".gz"):
            reader = Chem.ForwardSDMolSupplier(gzip.open(filename))
        else:
            reader = Chem.ForwardSDMolSupplier(filename)
        return reader
...
# def sdf_read_mol(filename):
#     txt = sdf_read_block(filename)
#     if txt == None:
#         return None
...
def rdk_read_mols(filename):
    ...
    if filename.endswith(".mol"):
        ...
        supplr = rdk_mol_supplier(filename)
